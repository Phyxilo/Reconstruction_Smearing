...
void OverlappingTracks()
{
  ...
  //sprintf(MCName, "/home/phyxilo/root/macros/Smearing/TrueMC/trueMC.root");
  sprintf(MCName, "/home/phyxilo/root/macros/Smearing/data2022.root");
  ...
  /*
  TLeaf *MCeID = MCTree->GetLeaf("Event");
  TLeaf *MCFlag = MCTree->GetLeaf("Flag");
  TLeaf *MCParID = MCTreetLeaf("ParID");
  TLeaf *MCTX = MCTree->GetLeaf("SlpTX");
  TLeaf *MCTY = MCTree->GetLeaf("SlpTY");
  TLeaf *MCT2 = MCTree->GetLeaf("SlpT2");
  */

  TLeaf *MCeID = MCTree->GetLeaf("trk_id");
    ...
    for(int j = 0; j < multNum; j++)
    {
      ...
      if (it != eIDListFull.end())
      {
        ...
        if (find(discardedTracks.begin(), discardedTracks.end(), index) == discardedTracks.end())
        {
          ...
          if (/*refFlag == 222 refParID == 1*/ true)
          {
            tX = refTrTX; tY = refTrTY; /*t2 = refTrT2;*/ t2 = sqrt(refTrTX*refTrTX + refTrTY*refTrTY);
